...
class Config:
    ...
    def flow_captcha_service_timeout_seconds(self) -> int:
        """Get Flow captcha service request timeout in seconds"""
        return int(self._config.get("captcha", {}).get("flow_captcha_service_timeout_seconds", 120))

    def set_flow_captcha_service_timeout_seconds(self, timeout_seconds: int):
        """Set Flow captcha service request timeout in seconds"""
        if "captcha" not in self._config:
            self._config["captcha"] = {}
        self._config["captcha"]["flow_captcha_service_timeout_seconds"] = int(timeout_seconds)
